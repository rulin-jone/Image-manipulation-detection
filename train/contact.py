...
class Window:
	def __init__(self, parent):
		...
		self.parent = parent
		self.root.geometry("%dx%d" % (1200, 400))  # 窗体尺寸
		center_window(self.root)                   # 将窗体移动到屏幕中央
		self.root.title("联系作者")                 # 窗体标题
	...
	def title(self, parent):
		...
		def label(frame, text, size, bold=False):
			return tk.Label(frame, text=text, bg="black", fg="white", height=12, font=_ft(size, bold))

		frame = tk.Frame(parent, bg="black")

		label(frame, "图像真伪鉴别系统", 20, True).pack(side=tk.LEFT, padx=10)
		# label(frame, "@", 16).pack(side=tk.LEFT, padx=100)
		label(frame, "@xxx", 16).pack(side=tk.LEFT, padx=0)
		label(frame, "提提意见", 16).pack(side=tk.LEFT, padx=200)
		label(frame, "", 12).pack(side=tk.RIGHT, padx=20)
		label(frame, "*********@qq.com", 16).pack(side=tk.RIGHT, padx=20)
		image_label(frame, "images\\customer_service.png", 40, 40, False).pack(side=tk.RIGHT)
