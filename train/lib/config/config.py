...
# 使用随机梯度下降（gradient decent）时最好固定随机数种子(rng_seed)，这样你的代码每次运行的结果都一致。这种做法在演示结果、比较算法或debug时特别有效。
...
# 训练用参数
tf.app.flags.DEFINE_float('weight_decay', 0.0005, "for regularization, weight decay")
tf.app.flags.DEFINE_float('learning_rate', 0.001, "learning rate")
tf.app.flags.DEFINE_float('momentum', 0.9, "momentum")  # 梯度下降的参数
...
tf.app.flags.DEFINE_integer('step_size', 30000, "Step size for reducing the learning rate")
# step_size只支持单步
tf.app.flags.DEFINE_integer('display', 10, "step of showing the training variables during training")

tf.app.flags.DEFINE_string('initializer', "truncated", "Network initialization parameters")
tf.app.flags.DEFINE_string('pretrained_model', "./data/imagenet_weights/vgg16.ckpt", "pretrained model")
...
tf.app.flags.DEFINE_integer('snapshot_iterations', 5000, "iteration to make snapshot")
